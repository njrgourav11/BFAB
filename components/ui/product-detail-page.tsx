...
export interface ProductDetailPageProps {
    product: Product;
    seller?: Seller; // Make optional
    breadcrumbs?: { label: string; href: string }[]; // Make optional
                ...
                <div className="flex flex-col">
                    ...
                    <div className="flex gap-2 my-6">
                        <Button
                            ...
                            onClick={() => onAddToCart && onAddToCart(1)}
                            disabled={product.inStock === false}
                        >
                            <ShoppingCart className="h-5 w-5" />
                            {product.inStock === false ? 'Out of Stock' : 'Add to Cart'}
                        </Button>
                        <Button size="lg" variant="outline" className="flex-1 gap-2"><Share2 className="h-5 w-5" /> Share</Button>
                    </div>
                    ...
                    {(product.tags || product.features) && (
                        <div className="flex flex-wrap gap-2 mb-6">
                            {product.tags?.map((tag, index) => (
                                <Badge key={`tag-${index}`} variant="secondary" className="text-sm font-normal py-1 px-3 gap-2">
                                    {tag.icon && <tag.icon className="h-4 w-4" />}
                                    {tag.label}
                                </Badge>
                            ))}
                            {product.features?.map((feature, index) => (
                                <Badge key={`feature-${index}`} variant="secondary" className="text-sm font-normal py-1 px-3 gap-2">

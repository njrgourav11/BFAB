    ...
    >
      <div className="container mx-auto px-3 sm:px-4">
        <div className="flex justify-between items-center h-16 gap-2">
          ...
          <Link
            href="/"
            className="flex items-center gap-2 sm:gap-3 hover:opacity-90 transition-opacity min-w-0 shrink"
          >
            <motion.div
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="relative w-7 h-7 sm:w-9 sm:h-9 md:w-10 md:h-10 shrink-0"
            >
              <Image src="/cat.svg" alt="BFAB Logo" fill className="object-contain" />
            </motion.div>
            ...
            <span className="flex flex-col justify-center leading-none min-w-0">
              ...
              <span
                className="hidden sm:block h-[2px] w-9 md:w-12 mt-1 rounded-full
                           bg-gradient-to-r from-pine/70 to-pine/10 dark:from-pine-tint/70 dark:to-pine-tint/10"
              />
            </span>
          </Link>
            ...
            {menuItems.map((item) => (
              <Link
                ...
                className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-gray-900
                           dark:hover:text-white hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg transition-all
          ...
          {/* Right Section - full desktop (lg and up) */}
          <div className="hidden lg:flex items-center gap-2 shrink-0">
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="p-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg
                         transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pine/40"
              aria-label="Search"
            >
              <Search size={20} />
            </motion.button>
            ...
            {user ? (
                ...
                >
                  {user.photoURL ? (
                    ...
                    <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center
                                    text-blue-600 dark:text-blue-400 font-bold border border-blue-200 dark:border-blue-800">
                ...
                <AnimatePresence>
                  {isProfileOpen && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: 10 }}
                      className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-900 rounded-xl shadow-xl
                                 border border-gray-200 dark:border-slate-800 overflow-hidden py-1"
                    >
                      ...
                      <Link
                        href="/profile"
                        className="flex items-center gap-2 px-4 py-3 text-sm text-gray-700 dark:text-gray-300
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-2 px-4 py-3 text-sm text-red-600
                                   hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors text-left"
                      >
              ...
              <div className="flex items-center gap-2">
                <Link
                  href="/login"
                  className="px-4 py-2 text-sm font-bold text-slate-700 dark:text-slate-300 hover:text-blue-600
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Link
                href="/cart"
                className="relative flex items-center gap-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900
                           px-4 py-2 rounded-lg font-semibold hover:bg-slate-800 dark:hover:bg-gray-100
                           transition-all shadow-sm"
              >
                <ShoppingCart size={20} />
                <span className="hidden xl:inline">Cart</span>
                {cartItemCount > 0 && (
                  <span className="absolute -top-2 -right-2 bg-blue-600 text-white text-xs font-bold rounded-full
          ...
          {/* Condensed actions — visible from smallest screens up through lg (fills the old dead zone) */}
          <div className="flex lg:hidden items-center gap-1 shrink-0">
            ...
            >
              <ShoppingCart size={20} />
              {cartItemCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-blue-600 text-white text-xs font-bold rounded-full
      ...
      {/* Mobile / condensed menu (up to lg) */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            <div className="container mx-auto px-4 py-4 space-y-1">
              {menuItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="block px-4 py-3 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800
              ...
              <div className="border-t border-gray-200 dark:border-slate-800 pt-3 mt-3 space-y-1">
                {user ? (
                  <>
                    ...
                    <Link
                      href="/profile"
                      className="flex items-center gap-2 px-4 py-3 text-gray-700 dark:text-gray-300 hover:bg-gray-100
                                 dark:hover:bg-slate-800 rounded-lg transition-all font-medium"
                      onClick={() => setIsOpen(false)}
                    >
                      <User size={18} />
                      <span>Profile</span>
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-2 px-4 py-3 text-red-600 hover:bg-red-50
                                 dark:hover:bg-red-900/10 rounded-lg transition-all font-medium text-left"
                    >
                      <LogOut size={18} />
                      <span>Logout</span>
                    </button>
                  ...
                  <div className="flex items-center gap-2 px-4">
                    <Link
                      href="/login"
                      className="flex-1 text-center px-4 py-2.5 text-sm font-bold text-slate-700 dark:text-slate-300
                                 border border-gray-200 dark:border-slate-700 rounded-lg hover:bg-gray-50
                    ...
                    <Link
                      href="/signup"
                      className="flex-1 text-center px-4 py-2.5 text-sm font-bold bg-blue-600 hover:bg-blue-700
